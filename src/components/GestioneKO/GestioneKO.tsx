...
class GestioneKO extends BaseComponent{
    ...
    public gestioneKOTableComponent=(props:any):JSX.Element=>{
      ...
      const [tableState, setTableState] = React.useState<ITableState>(this.initialTableState);
    
      const fetchData = (state: any, instance: any) => {
        if(props.reloaData || this.initialTableState.data.length===0){
            deselectAllCheckbox();
            getTableData(
              state,
              this.baseURL,
              ...
              axios,
              this.isMobileView
          ).then((tableData:ITableState)=>{this.initialTableState = tableData;setTableState(tableData)});
          }else{
            setTableState({
          ...
          if (rowInfo && rowInfo.row) {
            return {
              onDoubleClick: (e:any) => {
                ...
                props.history.push("/detailPageFase2",{data:rowInfo.original});
              }
            }
          }else{
            return {}
